...
class VpcSetUp(object):
    ...
    def set_up_internet_gw(self, v_id, v_setting):
        ...
        if not self.conn.get_all_internet_gateways(filters={'attachment.vpc-id': v_id.id}):
            ...
            internet_gw.add_tag("Name", v_setting['name'] + "_internet_gw")
    ...
    def set_up_route_table(self, v_id, sub_id, int_gw_id, sub_setting):
        ...
        time.sleep(3)
        # add tag
        route_table.add_tag("Name", sub_setting['subnet_name'] + "_rt")
        ...
        for dest in destination_list:
            ...
            while not rt_id:
                self.logger.debug("Route for %s created but waiting for it to be availabel", sub_setting['subnet_name'])

        self.logger.info("Route Table and Routes Created for subnet -- %s", sub_setting['subnet_name'])
        ...
        for vpc_item in vpcList:
            ...
            if not len(verify_vpc):
                ...
                while vpc_id.state != 'available':
                    self.logger.debug("VPC %s created but waiting for it to be available", current_vpc_setting['name'])
                self.logger.warning("Resquested VPC already exists! -- %s", vpc_id.id)
            # Add name tag
            vpc_id.add_tag("Name", current_vpc_setting['name'])
            ...
            for subnet_setting in current_vpc_setting['subnet_settings']:
                ...
                subnet_id.add_tag("Name", subnet_setting['subnet_name'])
    ...
    def delete_vpc(self, vpcSettings=[], vpcList=[], dryRun_flag=False):
        ...
        for vpc_name in vpcList:
            ...
            vpc_id = self.getTags.get_resource_id(vpc_name)
            ...
            if len(attached_sgs):
                for attached_sg in attached_sgs:
                    if str(attached_sg.id) != str(main_sgs[0].id):
                        ...
                        self.logger.debug("Deleted Security Group -- %s", attached_sg.id)
            ...
            if len(attached_internet_gw):
                for int_gw in attached_internet_gw:
                    ...
                    self.logger.debug("Deleted Internet Gateway-- %s", int_gw.id)
            ...
            if len(attached_subnets):
                for s_id in attached_subnets:
                    ...
                    self.logger.debug("Deleted Subnet -- %s", s_id.id)
            ...
            if len(attached_route_tables):
                for rt in attached_route_tables:
                    if str(rt.id) != str(main_route_tables[0].id):
                        ...
                        self.logger.debug("Deleted Route Table -- %s", rt.id)
